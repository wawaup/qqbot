...
if __name__ == "__main__":
    ...
    async def _run():
        ...
        if "--filter" in sys.argv:
            ...
            print("# 截取前 150 字交给模型，知识/资讯/技术才转发\n")
            for th in group_threads(tweets, TWITTER_USERNAME):
                ...
                if not should_forward(
                    ...
                    continue
                preview = preview_text(th)
                keep = await is_worth_forwarding(preview)
                label = "KEEP" if keep is True else "SKIP" if keep is False else "FAIL"
                shown = preview.replace("\n", " / ")[:70]
                print(f"{label} {root.id}  {shown}")
            return
        for t in tweets:
            flags = []
            if t.is_retweet:
